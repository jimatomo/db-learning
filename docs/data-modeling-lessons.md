...
以下は各 Lesson の `schema.sql` に対応する概念図です。relationship は実装上の FK 制約に合わせ、FK 列が `NOT NULL` なら参照先を **exactly one**（`||`）、nullable なら **zero or one**（`|o`）として描いています。
...
```mermaid
erDiagram
  ITERATIONS |o--o{ TODOS : contains
  PROJECTS |o--o{ TODOS : groups
  TODOS |o--o{ TODOS : parent_of
